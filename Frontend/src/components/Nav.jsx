...
function Icon({ type }) {
  const commonProps = { viewBox: '0 0 24 24', fill: 'none', stroke: 'currentColor', strokeWidth: '1.8', strokeLinecap: 'round', strokeLinejoin: 'round', 'aria-hidden': 'true' }
  ...
  if (type === 'balloon') {
    return (
      <svg {...commonProps}>
        <path d="M12 2c4.4 0 8 3.4 8 7.5S16.4 17 12 17 4 13.6 4 9.5 7.6 2 12 2Z" />
        <path d="M12 17v4M9.5 21h5" />
        <path d="M10.5 9c.8-.8 1.7-1.2 2.7-1.2" />
      </svg>
    )
  }
  ...
  return (
    <svg {...commonProps}>
      <path d="M5 9V7.5A2.5 2.5 0 0 1 7.5 5h9A2.5 2.5 0 0 1 19 7.5V9" />
      <path d="M4 10h16v7.5A2.5 2.5 0 0 1 17.5 20h-11A2.5 2.5 0 0 1 4 17.5V10Z" />
      <path d="M9 13h6" />
    </svg>
  )
}

function Nav({ user, onOpenLogin, onLogout, darkMode, onToggleDarkMode }) {
    ...
    <header className="top-header">
      <nav className="nav container" aria-label="Main navigation">
        ...
        <div className="nav-links" role="tablist" aria-label="Navigation categories">
          {navItems.map((item) => (
            <button key={item.id} type="button" className={`nav-item ${isActive(item.id === 'all' ? '/' : item.id === 'homes' ? '/host' : `/${item.id}`) ? 'active' : ''}`}>
              <Icon type={item.icon} />
      ...
      <div className="search-wrap container">
        <form className="search-panel" aria-label="Search for a stay">
          <label className="search-field">
            <span className="field-label">Where</span>
            <input type="text" placeholder="Search destinations" defaultValue="South Africa" />
          </label>
          <label className="search-field">
          ...
          <label className="search-field guests-field">
            ...
            <input type="text" placeholder="Add guests" defaultValue="2 guests" />
          </label>
          <button type="button" className="search-button" aria-label="Search stays">
